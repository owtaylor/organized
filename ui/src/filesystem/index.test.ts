...
describe("FileSystem", () => {
  ...
  describe("connection management", () => {
    ...
    it("should handle connection errors", async () => {
      ...
      await expect(fs.writeFile("test.txt", "old", "new"))
  ...
  describe("writeFile", () => {
    it("should send write_file command and return new content", async () => {
      const writePromise = fs.writeFile("TASKS.md", "old content", "new content");
    ...
    it("should handle unexpected response type", async () => {
      ...
      await expect(writePromise).rejects.toThrow("Unexpected response type: committed");
    });
  });
  ...
  describe("state listeners", () => {
    ...
    it("should remove listener when returned function is called", async () => {
      ...
      // Add two listeners
      const removeListener1 = fs.addStateListener((state) => states1.push(state));
      const removeListener2 = fs.addStateListener((state) => states2.push(state));
